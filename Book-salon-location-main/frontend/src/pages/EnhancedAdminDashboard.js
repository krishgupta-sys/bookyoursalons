...
import { DollarSign, Users, Calendar, Store, TrendingUp, BarChart3, Eye, LogOut, AlertTriangle, IndianRupee, Ban, CheckCircle, UserPlus, Activity, Percent } from 'lucide-react';
...
function EnhancedAdminDashboard() {
  ...
  const fetchAllData = async () => {
    ...
    try {
      ...
      try {
        const userAnalyticsRes = await axios.get(`${API}/admin/user-analytics`);
      ...
      // Fetch login stats
      try {
        const loginStatsRes = await axios.get(`${API}/admin/login-stats`);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        {/* Commission Section - 0% Commission */}
        <Card className="mb-8 border-green-200 bg-green-50" data-testid="commission-summary-card">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-green-700">
              <CheckCircle className="w-5 h-5" />
              0% Platform Commission
            </CardTitle>
            <CardDescription>Salon partners receive 100% of their earnings</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              <div className="p-4 bg-white rounded-lg">
                <p className="text-sm text-gray-600">Total Platform Revenue</p>
                <p className="text-2xl font-bold text-green-700">₹{safeNumber(analytics?.total_revenue || userAnalytics?.revenue?.total)}</p>
              </div>
              <div className="p-4 bg-white rounded-lg">
                <p className="text-sm text-gray-600">Salon Partner Earnings</p>
                <p className="text-2xl font-bold text-blue-700">₹{safeNumber(analytics?.total_revenue || userAnalytics?.revenue?.total)}</p>
                <p className="text-xs text-green-600">100% to partners</p>
              </div>
              <div className="p-4 bg-white rounded-lg">
                <p className="text-sm text-gray-600">Platform Fee</p>
                <p className="text-2xl font-bold text-gray-400">₹0</p>
                <p className="text-xs text-gray-500">0% commission</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Tabs defaultValue="bookings" className="space-y-4">
          <TabsList>
            ...
            <TabsTrigger value="users" data-testid="user-analytics-tab">User Analytics</TabsTrigger>
          </TabsList>
          ...
          {/* User Analytics Tab */}
          <TabsContent value="users" data-testid="user-analytics-content">
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                ...
                <CardDescription>Comprehensive view of platform user statistics</CardDescription>
              </CardHeader>
              <CardContent>
                {userAnalytics && (
                  <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      <div className="p-4 bg-blue-50 rounded-lg">
                        ...
                        <p className="text-2xl font-bold text-blue-700">{userAnalytics.users?.total_customers || 0}</p>
                      </div>
                      <div className="p-4 bg-purple-50 rounded-lg">
                        <div className="flex items-center gap-2 mb-2">
                          <Store className="w-4 h-4 text-purple-600" />
                          <span className="text-sm text-gray-600">Salon Partners</span>
                        </div>
                        <p className="text-2xl font-bold text-purple-700">{userAnalytics.users?.total_partners || 0}</p>
                      </div>
                      <div className="p-4 bg-green-50 rounded-lg">
                        ...
                        <p className="text-2xl font-bold text-green-700">{userAnalytics.users?.active_customers || 0}</p>
                      ...
                      <div className="p-4 bg-orange-50 rounded-lg">
                        ...
                        <p className="text-2xl font-bold text-orange-700">{userAnalytics.users?.new_this_week || 0}</p>
                      </div>
                    </div>
                      ...
                      <div className="p-4 bg-gray-50 rounded-lg">
                        ...
                        <div className="space-y-3">
                          <div className="flex justify-between items-center">
                            ...
                            <span className="font-bold text-lg">{userAnalytics.bookings?.total || 0}</span>
                          </div>
                          <div className="flex justify-between items-center">
                            <span className="text-gray-600">Completed</span>
                            <span className="font-semibold text-green-600">{userAnalytics.bookings?.completed || 0}</span>
                          </div>
                          <div className="flex justify-between items-center">
                            <span className="text-gray-600">Cancelled</span>
                            <span className="font-semibold text-red-600">{userAnalytics.bookings?.cancelled || 0}</span>
                          </div>
                          <div className="flex justify-between items-center pt-2 border-t">
                            <span className="text-gray-600 flex items-center gap-1">
                              <Percent className="w-3 h-3" /> Completion Rate
                            </span>
                            <span className="font-bold text-blue-600">{userAnalytics.bookings?.completion_rate || 0}%</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="p-4 bg-gray-50 rounded-lg">
                        ...
                        <div className="space-y-3">
                          <div className="flex justify-between items-center">
                            ...
                            <span className="font-bold text-lg">₹{(userAnalytics.revenue?.total || 0).toLocaleString()}</span>
                          </div>
                          <div className="flex justify-between items-center pt-2 border-t">
                            <span className="text-gray-600">Platform Earnings (10%)</span>
                            <span className="font-semibold text-green-600">₹{(userAnalytics.revenue?.platform_earnings || 0).toLocaleString()}</span>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    {/* Top Salons */}
                    {userAnalytics.top_salons?.length > 0 && (
                      <div className="p-4 bg-gray-50 rounded-lg">
                        <h4 className="font-semibold mb-4 flex items-center gap-2">
                          <TrendingUp className="w-4 h-4" />
                          ...
                          <table className="w-full text-sm">
                            <thead>
                              <tr className="border-b">
                                ...
                                <th className="text-left py-2">Salon Name</th>
                                <th className="text-right py-2">Bookings</th>
                                <th className="text-right py-2">Revenue</th>
                              </tr>
                            </thead>
                            <tbody>
                              {userAnalytics.top_salons.map((salon, idx) => (
                                <tr key={salon.salon_id} className="border-b">
                                  <td className="py-2 font-medium">{idx + 1}</td>
                                  <td className="py-2">{salon.salon_name}</td>
                                  <td className="py-2 text-right">{salon.bookings}</td>
                                  <td className="py-2 text-right font-medium">₹{salon.revenue}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
                    ...
                    <div className="grid md:grid-cols-3 gap-4">
                      <div className="p-4 bg-blue-50 rounded-lg text-center">
                        ...
                        <p className="text-3xl font-bold text-blue-700">{userAnalytics.users?.new_today || 0}</p>
                      </div>
                      <div className="p-4 bg-purple-50 rounded-lg text-center">
                        <p className="text-sm text-gray-600 mb-1">New Salons This Week</p>
                        <p className="text-3xl font-bold text-purple-700">{userAnalytics.users?.new_salons_week || 0}</p>
                      </div>
                      <div className="p-4 bg-green-50 rounded-lg text-center">
                        <p className="text-sm text-gray-600 mb-1">Total Registered Salons</p>
                        <p className="text-3xl font-bold text-green-700">{userAnalytics.users?.total_salons || 0}</p>
                      </div>
                    </div>
                    ...
                    <div className="p-4 bg-gray-50 rounded-lg">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div className="text-center p-3 bg-white rounded-lg">
                          <p className="text-2xl font-bold text-gray-800">{loginStats.total_users || 0}</p>
                          <p className="text-xs text-gray-500">Total Users</p>
                        </div>
                        <div className="text-center p-3 bg-white rounded-lg">
                          <p className="text-2xl font-bold text-green-600">{loginStats.today_logins || 0}</p>
                          <p className="text-xs text-gray-500">Today Logins</p>
                        </div>
                        <div className="text-center p-3 bg-white rounded-lg">
                          <p className="text-2xl font-bold text-blue-600">{loginStats.weekly_logins || 0}</p>
                          <p className="text-xs text-gray-500">Weekly Logins</p>
                        </div>
                        <div className="text-center p-3 bg-white rounded-lg">
                          <p className="text-2xl font-bold text-purple-600">{loginStats.monthly_logins || 0}</p>
                          <p className="text-xs text-gray-500">Monthly Logins</p>
                        </div>
                      </div>
                    </div>
                  </div>
