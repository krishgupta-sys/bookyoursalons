...
class SalonStatusUpdate(BaseModel):
    is_open: bool
    auto_close_enabled: Optional[bool] = True
...
class UserLoginTrack(BaseModel):
    phone: str
    name: Optional[str] = None
    firebase_uid: Optional[str] = None
    login_method: Optional[str] = "otp"  # otp, google

class PhotoGalleryUpload(BaseModel):
    salon_id: str
    photos: List[str]  # List of base64 encoded images

class FavoriteServiceCreate(BaseModel):
    customer_phone: str
    salon_id: str
    service_id: str
...
# Commission rate constant - 0% commission, salon partners receive 100% revenue
...
async def geocode_reverse(lat: float, lon: float):
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            headers = {"User-Agent": "BookYourSalons/1.0"}
            async with session.get(url, headers=headers) as response:
                if response.status == 200:
                    data = await response.json()
                    return data
                else:
                    raise HTTPException(status_code=response.status, detail="Geocoding failed")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Geocoding error: {str(e)}")

@api_router.get("/geocode/search")
async def geocode_search(q: str):
    """Proxy endpoint for Nominatim search to avoid CORS issues"""
    try:
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(url, headers=headers) as response:
                if response.status == 200:
                    ...
                    raise HTTPException(status_code=response.status, detail="Geocoding search failed")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Geocoding error: {str(e)}")
...
async def track_user_login(data: UserLoginTrack):
    ...
    if existing_user:
        ...
        await db.users.update_one(
            ...
            {"$set": {
                ...
                "firebase_uid": data.firebase_uid or existing_user.get("firebase_uid")
            },
            "$inc": {"login_count": 1}}
        )
    else:
    ...
    return {"message": "Login tracked", "phone": data.phone}

@api_router.get("/admin/login-stats")
async def get_login_stats():
    ...
    today_logins = await db.users.count_documents({"last_login_at": {"$gte": today_start}})
    weekly_logins = await db.users.count_documents({"last_login_at": {"$gte": week_ago}})
    monthly_logins = await db.users.count_documents({"last_login_at": {"$gte": month_ago}})
...
@api_router.post("/slot/lock")
async def lock_slot(data: SlotLock):
    ...
    existing_lock = await db.slot_locks.find_one({
    ...
    if existing_lock:
        if existing_lock.get("customer_phone") == data.customer_phone:
            # Same customer, extend lock
            await db.slot_locks.update_one(
                {"_id": existing_lock["_id"]},
                {"$set": {"expires_at": lock_expires.isoformat()}}
            )
            return {"locked": True, "message": "Lock extended", "expires_at": lock_expires.isoformat()}
        else:
            return {"locked": False, "message": "Slot is already locked by another customer"}
    ...
    if existing_booking:
        return {"locked": False, "message": "Slot is already booked"}
    ...
    return {"locked": True, "lock_id": lock_id, "expires_at": lock_expires.isoformat()}

@api_router.post("/slot/unlock/{lock_id}")
async def unlock_slot(lock_id: str):
    """Manually unlock a slot"""
    result = await db.slot_locks.delete_one({"lock_id": lock_id})
    if result.deleted_count > 0:
        return {"message": "Slot unlocked"}
    return {"message": "Lock not found"}

@api_router.delete("/slot/cleanup-expired")
async def cleanup_expired_locks():
    """Clean up expired slot locks (called periodically)"""
    now = datetime.now(timezone.utc).isoformat()
    result = await db.slot_locks.delete_many({"expires_at": {"$lt": now}})
...
async def approve_salon(salon_id: str):
    ...
    return {"message": "Salon approved with 30-day free trial"}
...
async def admin_block_salon(salon_id: str, reason: str = "manual"):
    ...
    return {"message": f"Salon {salon_id} blocked", "reason": reason}
...
async def get_admin_commission_summary():
    ...
    offline_pending = sum(ledger.get("total_pending", 0) for ledger in pending_ledgers)
    offline_overdue = sum(ledger.get("total_pending", 0) for ledger in overdue_ledgers)
    
    # Get overdue salons
    overdue_salon_ids = list(set(ledger["salon_id"] for ledger in overdue_ledgers))
...
async def create_review(review_data: ReviewCreate):
    ...
    salon = await db.salons.find_one({"salon_id": review_data.salon_id})
    if not salon:
        raise HTTPException(status_code=404, detail="Salon not found")
    ...
    existing_review = await db.reviews.find_one({
        ...
        "customer_phone": review_data.customer_phone
    })
    
    if existing_review:
        # Update existing review
        await db.reviews.update_one(
            {"salon_id": review_data.salon_id, "customer_phone": review_data.customer_phone},
            {"$set": {
    ...
    reviews = await db.reviews.find({"salon_id": review_data.salon_id}).to_list(1000)
    if reviews:
...
@api_router.get("/salon/{salon_id}/reviews")
async def get_salon_reviews(salon_id: str):
    """Get all reviews for a salon"""
    reviews = await db.reviews.find({"salon_id": salon_id}, {"_id": 0}).sort("created_at", -1).to_list(100)
    return reviews


# ==================== TRENDING & RECOMMENDED SALONS ====================

@api_router.get("/salons/trending")
async def get_trending_salons(lat: Optional[float] = None, lng: Optional[float] = None, limit: int = 10):
    """Get trending salons based on bookings and ratings"""
    # Get salons with bookings in the last 7 days
    week_ago = (datetime.now(timezone.utc) - timedelta(days=7)).isoformat()
    
    # Aggregate bookings by salon in last 7 days
    pipeline = [
        {"$match": {"created_at": {"$gte": week_ago}, "status": {"$in": ["confirmed", "completed"]}}},
        {"$group": {"_id": "$salon_id", "booking_count": {"$sum": 1}}},
        {"$sort": {"booking_count": -1}},
        {"$limit": limit * 2}  # Get more to filter
    ]
    
    trending_booking_data = await db.bookings.aggregate(pipeline).to_list(limit * 2)
    salon_ids = [item["_id"] for item in trending_booking_data]
    
    # Get salons
    salons = await db.salons.find(
        {"salon_id": {"$in": salon_ids}, "status": "approved"},
        {"_id": 0}
    ).to_list(limit * 2)
    
    # Create lookup dict
    booking_counts = {item["_id"]: item["booking_count"] for item in trending_booking_data}
    
    # Calculate trending score (bookings * 0.7 + rating * 0.3)
    for salon in salons:
        bookings = booking_counts.get(salon["salon_id"], 0)
        rating = salon.get("rating", 4.0)
        salon["trending_score"] = bookings * 0.7 + rating * 10 * 0.3
        salon["recent_bookings"] = bookings
    
    # Sort by trending score
    salons.sort(key=lambda x: x.get("trending_score", 0), reverse=True)
    
    return salons[:limit]

@api_router.get("/salons/recommended")
async def get_recommended_salons(customer_phone: str, lat: Optional[float] = None, lng: Optional[float] = None, limit: int = 10):
    """Get recommended salons for a customer based on history"""
    # Get customer's past bookings
    past_bookings = await db.bookings.find(
        {"customer_phone": customer_phone},
        {"_id": 0, "salon_id": 1, "service_name": 1}
    ).to_list(100)
    
    if not past_bookings:
        # Return popular salons for new users
        return await get_trending_salons(lat, lng, limit)
    
    # Get frequently booked services
    service_counts = {}
    visited_salons = set()
    for booking in past_bookings:
        service = booking.get("service_name", "").lower()
        if service:
            service_counts[service] = service_counts.get(service, 0) + 1
        visited_salons.add(booking.get("salon_id"))
    
    # Find top services
    top_services = sorted(service_counts.items(), key=lambda x: x[1], reverse=True)[:3]
    service_keywords = [s[0] for s in top_services]
    
    # Find similar salons with good ratings
    query = {
        "status": "approved",
        "salon_id": {"$nin": list(visited_salons)},  # Exclude already visited
        "rating": {"$gte": 3.5}
    }
    ...
    if lat and lng:
        query["location"] = {
            "$geoWithin": {
                "$centerSphere": [[lng, lat], 10 / 6371.0]  # 10km radius
            }
        }
    
    recommended = await db.salons.find(query, {"_id": 0}).sort("rating", -1).to_list(limit)
...
async def smart_search(query: str, lat: Optional[float] = None, lng: Optional[float] = None, limit: int = 20):
    """Smart search across salons, services, and areas"""
    ...
    name_results = await db.salons.find({
        "status": "approved",
        "salon_name": {"$regex": query, "$options": "i"}
    }, {"_id": 0}).limit(limit).to_list(limit)
    results["salons"] = name_results
    
    # Search by service
    service_results = await db.salons.find({
        "status": "approved",
        "services.name": {"$regex": query, "$options": "i"}
    }, {"_id": 0}).limit(limit).to_list(limit)
    results["by_service"] = [s for s in service_results if s not in results["salons"]]
    
    # Search by area
    area_results = await db.salons.find({
        "status": "approved",
        "area": {"$regex": query, "$options": "i"}
    }, {"_id": 0}).limit(limit).to_list(limit)
    results["by_area"] = [s for s in area_results if s not in results["salons"] and s not in results["by_service"]]
    
    # Combine and rank
    all_results = results["salons"] + results["by_service"] + results["by_area"]
    
    # Add search relevance score
    for salon in all_results:
        score = 0
        if query_lower in salon.get("salon_name", "").lower():
            score += 10
        if any(query_lower in s.get("name", "").lower() for s in salon.get("services", [])):
            score += 5
        if query_lower in salon.get("area", "").lower():
            score += 3
        salon["search_score"] = score + salon.get("rating", 4.0)
...
@api_router.post("/booking/check-spam")
async def check_spam_booking(data: SpamCheck):
    """Check if customer can make a new booking (spam protection)"""
    customer_phone = data.customer_phone
    now = datetime.now(timezone.utc)
    ...
    if recent_bookings >= 3:
        return {
            "allowed": False,
            "message": "Too many bookings! Please wait 5 minutes before booking again.",
            "recent_count": recent_bookings
        }
    
    return {"allowed": True, "message": "Booking allowed", "recent_count": recent_bookings}


# ==================== SALON HOURS & STATUS CONTROL ====================

@api_router.patch("/salon/{salon_id}/hours")
async def update_salon_hours(salon_id: str, hours_data: SalonHoursUpdate):
    ...
    update_fields = {
        "opening_time": hours_data.opening_time,
        "closing_time": hours_data.closing_time,
        "updated_at": datetime.now(timezone.utc).isoformat()
    }
    
    if hours_data.working_days:
        update_fields["working_days"] = hours_data.working_days
    
    await db.salons.update_one({"salon_id": salon_id}, {"$set": update_fields})
    
    return {"message": "Operating hours updated"}

@api_router.patch("/salon/{salon_id}/toggle-status")
async def toggle_salon_status(salon_id: str, status_data: SalonStatusUpdate):
    """Toggle salon open/closed status"""
    salon = await db.salons.find_one({"salon_id": salon_id})
    if not salon:
        raise HTTPException(status_code=404, detail="Salon not found")
    
    current_status = "open" if status_data.is_open else "closed"
    
    await db.salons.update_one(
        {"salon_id": salon_id},
        {"$set": {
            "current_status": current_status,
            "auto_close_enabled": status_data.auto_close_enabled,
            "status_updated_at": datetime.now(timezone.utc).isoformat()
        }}
    )
    
    return {"message": f"Salon is now {current_status}", "status": current_status}

@api_router.post("/salon/{salon_id}/check-auto-close")
async def check_auto_close(salon_id: str):
    """Check if salon should auto-close after last slot"""
    salon = await db.salons.find_one({"salon_id": salon_id})
    if not salon:
        raise HTTPException(status_code=404, detail="Salon not found")
    
    if not salon.get("auto_close_enabled", True):
        return {"auto_closed": False, "reason": "Auto-close disabled"}
    ...
    avg_time = salon.get("avg_service_time", 30)
    opening_time = salon.get("opening_time", "09:00")
    closing_time = salon.get("closing_time", "20:00")
    
    # Calculate total slots
    open_hour = int(opening_time.split(":")[0])
    close_hour = int(closing_time.split(":")[0])
    hours = close_hour - open_hour
    slots_per_hour = staff_count * (60 // avg_time)
    total_slots = hours * slots_per_hour
    
    if len(bookings) >= total_slots:
        await db.salons.update_one(
            {"salon_id": salon_id},
            {"$set": {"current_status": "fully_booked", "status_updated_at": datetime.now(timezone.utc).isoformat()}}
        )
        return {"auto_closed": True, "reason": "Fully booked for today"}
    
    return {"auto_closed": False, "bookings": len(bookings), "total_slots": total_slots}


# ==================== PHOTO GALLERY ====================

@api_router.post("/salon/{salon_id}/gallery")
async def upload_gallery_photos(salon_id: str, photos_data: PhotoGalleryUpload):
    """Upload multiple photos to salon gallery"""
    salon = await db.salons.find_one({"salon_id": salon_id})
    if not salon:
        raise HTTPException(status_code=404, detail="Salon not found")
    
    existing_gallery = salon.get("photo_gallery", [])
    ...
    updated_gallery = (existing_gallery + new_photos)[:20]
    
    await db.salons.update_one(
        {"salon_id": salon_id},
        {"$set": {"photo_gallery": updated_gallery}}
    )
    
    return {"message": f"Added {len(new_photos)} photos to gallery", "total_photos": len(updated_gallery)}

@api_router.delete("/salon/{salon_id}/gallery/{photo_id}")
async def delete_gallery_photo(salon_id: str, photo_id: str):
    """Delete a photo from gallery"""
    await db.salons.update_one(
        {"salon_id": salon_id},
        {"$pull": {"photo_gallery": {"photo_id": photo_id}}}
    )
...
@api_router.post("/customer/favorite-service")
async def add_favorite_service(data: FavoriteServiceCreate):
    """Add a service to customer favorites"""
    existing = await db.favorite_services.find_one({
        "customer_phone": data.customer_phone,
        "salon_id": data.salon_id,
    ...
    if existing:
        await db.favorite_services.delete_one({
            "customer_phone": data.customer_phone,
            "salon_id": data.salon_id,
            "service_id": data.service_id
        })
        return {"favorited": False, "message": "Service removed from favorites"}
    
    await db.favorite_services.insert_one({
        "customer_phone": data.customer_phone,
        "salon_id": data.salon_id,
        "service_id": data.service_id,
        "created_at": datetime.now(timezone.utc).isoformat()
    })
    return {"favorited": True, "message": "Service added to favorites"}

@api_router.get("/customer/{phone}/favorite-services")
async def get_favorite_services(phone: str):
    """Get customer's favorite services"""
    favorites = await db.favorite_services.find({"customer_phone": phone}, {"_id": 0}).to_list(50)
    return favorites


# ==================== REMINDERS ====================

@api_router.get("/bookings/upcoming-reminders/{phone}")
async def get_upcoming_reminders(phone: str):
    """Get bookings that need reminders (within 1 hour)"""
    now = datetime.now(timezone.utc)
    ...
    for booking in bookings:
        try:
            booking_time = booking.get("slot_time", "")
            if booking_time:
                booking_datetime = datetime.strptime(f"{today} {booking_time}", "%Y-%m-%d %H:%M")
                booking_datetime = booking_datetime.replace(tzinfo=timezone.utc)
                time_diff = (booking_datetime - now).total_seconds() / 60  # Minutes
                
                if 0 < time_diff <= 60:  # Within 1 hour
                    reminders.append({
                        **booking,
                        "minutes_away": int(time_diff),
                        "is_urgent": time_diff <= 5
                    })
        except:
            continue
...
@api_router.get("/customer/{phone}/service-reminders")
async def get_service_reminders(phone: str):
    """Get repeat service reminders based on past bookings"""
    # Get completed bookings from more than 30 days ago
    ...
    for booking in past_bookings:
        ...
        if service and service not in service_last_visit:
            service_last_visit[service] = {
                "service_name": service,
                "salon_name": booking.get("salon_name"),
                "salon_id": booking.get("salon_id"),
                "last_visit": booking.get("booking_date"),
                "days_since": (datetime.now(timezone.utc) - datetime.fromisoformat(booking.get("created_at", "").replace("Z", "+00:00"))).days
            }
    
    reminders = [v for v in service_last_visit.values() if v["days_since"] >= 30]
    return reminders


# ==================== CUSTOMER ANALYTICS ====================

@api_router.get("/customer/{phone}/visit-history")
async def get_customer_visit_history(phone: str):
    """Get customer's complete visit history"""
    bookings = await db.bookings.find(
        {"customer_phone": phone},
        {"_id": 0}
    ...
    for booking in bookings:
        salon_id = booking.get("salon_id")
        if salon_id not in salon_visits:
            salon_visits[salon_id] = {
                "salon_id": salon_id,
                "salon_name": booking.get("salon_name"),
                "visit_count": 0,
                "total_spent": 0,
        ...
        salon_visits[salon_id]["visit_count"] += 1
        if booking.get("payment_status") == "paid":
            salon_visits[salon_id]["total_spent"] += booking.get("service_price", 0)
        if not salon_visits[salon_id]["last_visit"]:
            salon_visits[salon_id]["last_visit"] = booking.get("booking_date")
        if booking.get("service_name") not in salon_visits[salon_id]["services"]:
            salon_visits[salon_id]["services"].append(booking.get("service_name"))
    
    return {
        "total_bookings": len(bookings),
        "salons_visited": len(salon_visits),
        "salon_breakdown": list(salon_visits.values()),
        "is_repeat_customer": len(bookings) > 1
    }


# ==================== SALON DASHBOARD ANALYTICS ====================

@api_router.get("/salon/{salon_id}/dashboard-analytics")
async def get_salon_dashboard_analytics(salon_id: str):
    """Get comprehensive analytics for salon dashboard"""
    salon = await db.salons.find_one({"salon_id": salon_id}, {"_id": 0})
    if not salon:
        raise HTTPException(status_code=404, detail="Salon not found")
    ...
    # Today's stats
    today_bookings = [b for b in all_bookings if b.get("booking_date") == today]
    today_revenue = sum(b.get("service_price", 0) for b in today_bookings if b.get("payment_status") == "paid")
    
    # Week stats
    week_bookings = [b for b in all_bookings if b.get("created_at", "") >= week_ago]
    week_revenue = sum(b.get("service_price", 0) for b in week_bookings if b.get("payment_status") == "paid")
    
    # Month stats
    month_bookings = [b for b in all_bookings if b.get("created_at", "") >= month_ago]
    month_revenue = sum(b.get("service_price", 0) for b in month_bookings if b.get("payment_status") == "paid")
    
    # Popular services
    service_counts = {}
    for b in all_bookings:
        service = b.get("service_name", "Unknown")
        service_counts[service] = service_counts.get(service, 0) + 1
    ...
    if subscription.get("expires_at"):
        ...
        trial_info = {
            ...
            "status": salon.get("subscription_status", "inactive"),
    ...
    return {
        ...
        "today": {
            ...
            "pending": len([b for b in today_bookings if b.get("status") == "pending_approval"])
        },
        "week": {
        ...
        "all_time": {
            ...
            "revenue": sum(b.get("service_price", 0) for b in all_bookings if b.get("payment_status") == "paid")
        },
        "customers": {
        ...
        "peak_hours": [{"hour": f"{h[0]}:00", "count": h[1]} for h in peak_hours],
...
async def get_admin_user_analytics():
    ...
    # User counts
    total_customers = await db.users.count_documents({"role": "customer"})
    total_partners = await db.salon_partners.count_documents({})
    total_salons = await db.salons.count_documents({})
    ...
    new_salons_week = await db.salons.count_documents({
        "created_at": {"$gte": week_ago.isoformat()}
    })
    ...
    top_salons_pipeline = [
        ...
        {"$group": {"_id": "$salon_id", "count": {"$sum": 1}, "revenue": {"$sum": "$service_price"}}},
        {"$sort": {"count": -1}},
        {"$limit": 10}
    ]
    top_salons_data = await db.bookings.aggregate(top_salons_pipeline).to_list(10)
    
    # Get salon names
    top_salons = []
    for item in top_salons_data:
        salon = await db.salons.find_one({"salon_id": item["_id"]}, {"_id": 0, "salon_name": 1})
        if salon:
            top_salons.append({
                "salon_id": item["_id"],
    ...
    return {
        ...
        "revenue": {
            "total": total_revenue,
            "platform_earnings": round(total_revenue * 0.10, 2)
        },
        "top_salons": top_salons
    }


# ==================== FAKE REGISTRATION PROTECTION ====================

@api_router.post("/salon/validate-registration")
async def validate_salon_registration(data: dict):
    """Validate salon registration for fake registration protection"""
    phone = data.get("phone", "")
    ...
    # Check if phone already registered
    existing_phone = await db.salons.find_one({"phone": phone})
    if existing_phone:
        warnings.append("Phone number already registered with another salon")
    
    # Check if similar salon name exists in same area
    similar_name = await db.salons.find_one({
        "salon_name": {"$regex": f"^{salon_name}$", "$options": "i"},
        "area": {"$regex": area, "$options": "i"}
    })
    if similar_name:
        warnings.append("A salon with similar name exists in this area")
    
    # Check registration rate from same IP (requires middleware)
    # For now, check by phone prefix
    phone_prefix = phone[:6] if len(phone) >= 6 else phone
    recent_registrations = await db.salons.count_documents({
        "phone": {"$regex": f"^{phone_prefix}"},
        "created_at": {"$gte": (datetime.now(timezone.utc) - timedelta(days=1)).isoformat()}
    })
    if recent_registrations >= 3:
        warnings.append("Multiple registrations detected from similar phone numbers")
