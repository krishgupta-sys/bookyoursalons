...
function LocationPickerModal({ isOpen, onClose, onLocationSelect, initialLocation, title = "Choose Your Location" }) {
  ...
  // Reverse geocode only when position changes and is different from last
  useEffect(() => {
    if (position) {
      const posKey = `${position.lat.toFixed(4)},${position.lng.toFixed(4)}`;
      if (lastGeocodedRef.current !== posKey) {
        lastGeocodedRef.current = posKey;
  ...
  const reverseGeocode = async (lat, lng) => {
    try {
      // Use relative API path for Firebase Functions
